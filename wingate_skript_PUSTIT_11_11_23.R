...
srovnani <- winDialog("yesno", "Pøidat srovnání s pøedchozími výsledky?")
...
if (srovnani == "YES") {
  winDialog("ok", "Vyberte složku pro nejnovìjši srovnání")
  comparison.path <-  rstudioapi::selectDirectory(path = getwd(), caption = "Vyberte slozku pro nejnovejsi srovnani")
  file.list.compar <- list.files(path = comparison.path, pattern = '*.txt')
  dalsi_srov <- winDialog("yesno", "Pøidat další srovnání?")
  if (dalsi_srov == "YES") {
    comparison.path_2 <- rstudioapi::selectDirectory(path = getwd(), caption = "Vyberte slozku pro dalsi srovnani")
...
if(exists("file.list.compar_2.bez")) {
  if (srovnani == "YES" & id %in% file.list.compar_2.bez) {
    ...
    if (compare.wingate_2$Elapsed.time.total..h.mm.ss.hh.[1] > 5) {
    ...
    s5 <- head(which(compare.wingate_2$Elapsed.time.total..h.mm.ss.hh. >= 5), n=1)
    s10 <- head(which(compare.wingate_2$Elapsed.time.total..h.mm.ss.hh. >= 10), n=1)
    s15 <- head(which(compare.wingate_2$Elapsed.time.total..h.mm.ss.hh. >= 15), n=1)
    s20 <- head(which(compare.wingate_2$Elapsed.time.total..h.mm.ss.hh. >= 20), n=1)
    s25 <- head(which(compare.wingate_2$Elapsed.time.total..h.mm.ss.hh. >= 25), n=1)
    s30 <- length(compare.wingate_2$Elapsed.time.total..h.mm.ss.hh.)
...
if(exists("file.list.compar_2.bez")) {
  if (srovnani == "YES" & id %in% file.list.compar_2.bez) {
    ...
    df2$Pmax_W <- max(compare.wingate_2$Power..W.)
    df2$PP_BW <- round(max(compare.wingate_2$Power..W.)/antropo$Weight[length(antropo$Weight)-2],2)
    df2$Pmin_W <- min(compare.wingate_2$Power..W.[round((length(compare.wingate_2$Power..W.)/2),0):length(compare.wingate_2$Power..W.)])
    df2$Average_power_W <- round(mean(compare.wingate_2$Power..W.),1)
    df2$Pdrop_W <- df2$Pmax_W - df2$Pmin_W
    df2$Pmax_5s <- round(max(compare.wingate_2$RM5_Power, na.rm = T),1)
    df2$IU_pct <- round((max(na.omit(compare.wingate_2$RM5_Power))-round(min(na.omit(compare.wingate_2$RM5_Power)),1))/max(na.omit(compare.wingate_2$RM5_Power))*100,1)
    df2$Work_kJ <- round(mean(compare.wingate_2$AvP_dopocet*30),0)/1000
    df2$HR_max_BPM <- max(compare.wingate_2$Heart.rate..bpm., na.rm =T)
...
if (srovnani == "YES" & id %in% file.list.compar.bez) {
  ...
  df2$Pmax_W <- append(na.omit(df2$Pmax_W), max(compare.wingate$Power..W.))
  df2$PP_BW <- append(na.omit(df2$PP_BW), round(max(compare.wingate$Power..W.)/antropo$Weight[length(antropo$Weight)-1],2))
  df2$Pmin_W <- append(na.omit(df2$Pmin_W), min(compare.wingate$Power..W.[round((length(compare.wingate$Power..W.)/2),0):length(compare.wingate$Power..W.)]))
  df2$Average_power_W <- append(na.omit(df2$Average_power_W), round(mean(compare.wingate$Power..W.),1))
  df2$Pdrop_W <- append(na.omit(df2$Pdrop_W), (df2$Pmax_W[nrow(df2)] - df2$Pmin_W[nrow(df2)]))
  df2$Pmax_5s <- append(na.omit(df2$Pmax_5s), round(max(compare.wingate$RM5_Power, na.rm = T),1))
  df2$IU_pct <- append(na.omit(df2$IU_pct), round((max(na.omit(compare.wingate$RM5_Power))-round(min(na.omit(compare.wingate$RM5_Power)),1))/max(na.omit(compare.wingate$RM5_Power))*100,1))
  df2$Work_kJ <- append(na.omit(df2$Work_kJ), round(mean(compare.wingate$AvP_dopocet*30),0)/1000)
  df2$HR_max_BPM <- append(na.omit(df2$HR_max_BPM), max(compare.wingate$Heart.rate..bpm., na.rm =T))
  df2$La_max_mmol_l <- append(na.omit(df2$La_max_mmol_l), antropo$LA[length(antropo$LA)-1])
  df2[nrow(df2) + 1,] <- NA
  }
}
...
if(exists("file.list.compar_2.bez")) {
  ...
  plot1 <- ggplot2::ggplot(df, aes(x = Elapsed.time.total..h.mm.ss.hh., y = Power..W.)) + 
    ...
    scale_colour_identity(guide = "legend", breaks = c("black", "grey", "blue", "orange", "darkmagenta"), labels = c("black"=paste("vyhlazená data,", datum_mer, sep = " "), "blue"="prùmìr mìøení","grey"="hrubá data","orange"=format(as.Date(antropo$Date_measurement[length(antropo$Date_measurement)-1]), "%d/%m/%Y"),"darkmagenta"=format(as.Date(antropo$Date_measurement[length(antropo$Date_measurement)-2]), "%d/%m/%Y"))) + guides(color = guide_legend(override.aes = list(linetype = c("solid", "dashed", "dashed", "solid", "dashed")))) + 
...
} else if (exists("file.list.compar_2.bez")) {
  if (tri_graf == "NO") {
  plot1 <- ggplot2::ggplot(df, aes(x = Elapsed.time.total..h.mm.ss.hh., y = Power..W.)) + 
    theme_classic()  + 
    theme(text = element_text(size = 30),
    ...
    geom_line(data = df, aes(y=y, x = Elapsed.time.total..h.mm.ss.hh., color = "black", linetype = "solid"), lwd = 0.9) + 
    scale_linetype_manual(values = c('dashed', 'solid')) +
    scale_colour_identity(guide = "legend", breaks = c("black", "grey", "blue", "orange"), labels = c("black"=paste("vyhlazená data,", datum_mer, sep = " "), "blue"="prùmìr mìøení","grey"="hrubá data","orange"=format(as.Date(antropo$Date_measurement[length(antropo$Date_measurement)-1]), "%d/%m/%Y"))) + guides(color = guide_legend(override.aes = list(linetype = c("solid", "dashed", "dashed", "solid")))) + 
    guides(linetype = "none") + 
    xlab(expression("Èas (s)")) + 
    ylab("Výkon (W)") + 
    ggtitle("Anaerobní Wingate test - 30 s")  + 
...
} else if(exists("file.list.compar.bez")) {
  if (srovnani == "YES" & id %in% file.list.compar.bez) {
    plot1 <- ggplot2::ggplot(df, aes(x = Elapsed.time.total..h.mm.ss.hh., y = Power..W.)) + 
      theme_classic()  + 
      theme(text = element_text(size = 30),
      ...
      geom_line(aes(y = mean(Power..W.), color= "blue", linetype = "dashed"), lwd = 0.9) +
      scale_linetype_manual(values = c('dashed', 'solid')) +
      scale_colour_identity(guide = "legend", labels = c(paste("vyhlazená data,", datum_mer, sep = " "), "prùmìr mìøení", "hrubá data", format(as.Date(antropo$Date_measurement[length(antropo$Date_measurement)-1]), "%d/%m/%Y"))) + guides(color = guide_legend(override.aes = list(linetype = c("solid", "solid", "dashed", "solid")))) + 
      guides(linetype = "none") + 
      xlab(expression("Èas (s)")) + 
      ylab("Výkon (W)") + 
      ggtitle("Anaerobní Wingate test - 30 s")  + 
  ...
  plot1 <- ggplot2::ggplot(df, aes(x = Elapsed.time.total..h.mm.ss.hh., y = Power..W.)) + 
    ...
    geom_line(aes(y = mean(Power..W.), color= "blue", linetype = "dashed"), lwd = 0.9) +
    scale_linetype_manual(values = c('dashed', 'solid')) +
    scale_colour_identity(guide = "legend", labels = c(paste("vyhlazená data,", datum_mer, sep = " "), "prùmìr mìøení", "hrubá data")) + guides(color = guide_legend(override.aes = list(linetype = c("solid", "dashed", "dashed")))) + 
    guides(linetype = "none") + 
    xlab(expression("Èas (s)")) + 
    ylab("Výkon (W)") + 
    ggtitle("Anaerobní Wingate test - 30 s")  + 
