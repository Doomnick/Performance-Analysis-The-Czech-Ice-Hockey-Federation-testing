...
wd <- "C:/Users/DKolinger/Documents/R/Wingate"
...
if (srovnani == "YES") {
  ...
  comparison.path <-  rstudioapi::selectDirectory(path = getwd(), caption = "Vyberte slozku pro nejnovější srovnání")
...
for(i in 1:length(file.list)) {
  ...
  # for (m in 1:length(df$Elapsed.time.total..h.mm.ss.hh.)) {
  # df$Elapsed.time.total..h.mm.ss.hh.[m] <- 30/length(df$Elapsed.time.total..h.mm.ss.hh.)*m
  # }
  ...
  df1$`Wingate test` <- `length<-`(c("Maximální výkon", "Minimální výkon", "Pokles výkonu", "Celková práce", "Index Únavy", "Celkový počet otáček", "Laktát", "Maximální tepová frekvence"),nrow(df1))
  ...
  #tabulka report 
  table1 <- df1 %>% gt() %>% 
    tab_header(title = fullname, subtitle = paste("Datum měření: ", format(as.Date(datum.mer), "%d/%m/%Y"), sep = "")) %>% 
