...
if ("TRUE" %in% duplikaty) {
  ...
  file.list.an.bez[which(duplikaty=="TRUE")]
} 
...
if (an.input == "batch") {
  ...
  age <- na.omit(antropo$Age[antropo$ID == id])
} else {
  ...
  age <- na.omit(tail(antropo$Age, n =1))
}
...
if (srovnani == "A" & id %in% file.list.compar.bez) {
  ...
  df2$Work_kJ <- round(mean(compare.wingate$Work..J.*30),0)/1000
