...
# Raw data Error reporting
  ...
  if ("FALSE" %in% kontrola || "FALSE" %in% kontrola.s) {
    ...
    if (input_2 == "NO") {
      ...
      if ("FALSE" %in% kontrola.s2) {
        spatne.compar_2 <- setdiff(file.list.bez, file.list.compar_2.bez)
      }
    }
  }
...
#### Spiroergomatery values  ####
  ...
  df2$HR_max_BPM <- as.numeric(append(na.omit(df2$HR_max_BPM), max(df$Heart.rate..bpm., na.rm =T)))
...
#### Wingate plots ####
  ...
  if(exists("file.list.compar_2.bez")) {
    ...
    else if (tri_graf == "NO" | id %in% spatne.compar_2) {
      plot1 <- ggplot2::ggplot(df, aes(x = Elapsed.time.total..h.mm.ss.hh., y = Power..W.)) + 
        theme_classic()  + 
        theme(text = element_text(size = 30),
              panel.grid.major = element_line(color = "grey90",
...
#### Export of Reporting tables ####
 ...
 #### Radarchart ####
  ...
  if (dotaz_spiro == "YES") {
    ...
    data_radar <- cbind( `VO2Max (l/mil/kg) %` = hg_Vo2max, data_radar, `Výkon VO2Max (W/kg) %` = hg_VO2vykon)
  }
